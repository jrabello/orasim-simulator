...
export class RedoLogFiles{
    ...
    async storeData(blocks: DataBlock[]){
        ...
        if(this.numBlocksInserted >= 3){
            ...
            await new Delay(3000).sleep()
            Orasim.getSqlConsole().addMsg(new SqlConsoleMsgInfo('< ARCH > REDO Grupo '+selectorPlusOne+' status..: EM CÓPIA'))        
            await new Delay(3000).sleep()
            Orasim.getSqlConsole().addMsg(new SqlConsoleMsgInfo('< ARCH > Copiando o Redo Log File: Grupo '+selectorPlusOne))        
            await new Delay(3000).sleep()            
            
            //animacao salvando archive
            Orasim.getSqlConsole().addMsg(new SqlConsoleMsgInfo('< ARCH > Salvando como Archived Log File: ARC_0'+ ++this.incrementer))        
            await new Delay(3000).sleep()
            Orasim.getAnimation().animBlinkTwoElements("#arcn", "#oracle-log-container", 5000)                                
            await new Delay(3000).sleep()            
            $("#arc0"+this.incrementer).removeClass("hiddenArc")            
            Orasim.getSqlConsole().addMsg(new SqlConsoleMsgInfo('< ARCH > Archive ARC_0'+this.incrementer+' salvo com sucesso!'))
            await new Delay(3000).sleep()            
            
            //swithing to another group
            Orasim.getSqlConsole().addMsg(new SqlConsoleMsgInfo('< LGWR > REDO Grupo '+selectorPlusOne+' status..: LIBERADO'))        
            await new Delay(3000).sleep()            
            this.selectorIndex ^= 1
            this.switchGroups()
            selectorPlusOne = this.selectorIndex+1

            Orasim.getSqlConsole().addMsg(new SqlConsoleMsgInfo('< LGWR > REDO Grupo '+ selectorPlusOne +' arquivos: '+ this.discoA[this.selectorIndex] +' e '+this.discoB[this.selectorIndex]+' '))
            await new Delay(3000).sleep()
